...
class Less_ {
 public:
  ...
  template<typename Type>
  bool operator()(Type x) { return x < bound_; }
 private:
  U bound_;
};
...
class IndexByCount_ {
 public:
  ...
  template<typename CountType, typename SumType>
  bool operator()(CasAggr<CountType, SumType> x) {
    if (i_ < x.count) return true;
    i_ -= x.count;
    return false;
  }
 private:
...
class NonzeroAnd_ {
 public:
  ...
  template<typename Type>
  bool operator()(Type x) { return x & mask_; }
 private:
  U mask_;
};
...
class DwLess_ {
 public:
  ...
  template<typename Type>
  bool operator()(DpAggr<Type> x) { return x.dw(dir_) < bound_; }
 private:
  int dir_;
  U bound_;
};
