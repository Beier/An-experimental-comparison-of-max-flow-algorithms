...
inline void WithValue<Group, Base>::AddToProperAnc(
  ...
  AddToSeq(root_cut.excl_part(), delta);
}
...
inline void WithValue<Group, Base>::SubtractFromProperAnc(
    ...
    typename Group::Type delta) {
  ScopedCutDirOf<Node> root_cut(kRoot, node);
  SubtractFromSeq(root_cut.excl_part(), delta);
}
...
template<typename Group, typename Base>
template<typename Node>
inline typename Group::Type WithDescValue<Group, Base>::Value(Node* node) {
  typedef WithDescValue Self;
  assert(node);
...
template<typename Group, typename Base>
template<typename Node>
inline void WithDescValue<Group, Base>::AddToTree(Node* node,
                                                  typename Group::Type delta) {
  typedef WithDescValue Self;
  if (!node) return;
...
template<typename Group, typename Base>
template<typename Node>
inline void WithDescValue<Group, Base>::SubtractFromTree(
    Node* node,
    typename Group::Type delta) {
  typedef WithDescValue Self;
  if (!node) return;
...
template<typename Group, typename Base>
template<typename Node>
inline void WithDescValue<Group, Base>::AddToDesc(Node* node,
                                                  typename Group::Type delta) {
  ScopedCutDirOf<Node> root_cut(kRoot, node);
  AddToTree(node, delta);
}

template<typename Group, typename Base>
template<typename Node>
inline void WithDescValue<Group, Base>::SubtractFromDesc(
    Node* node,
    typename Group::Type delta) {
  ScopedCutDirOf<Node> root_cut(kRoot, node);
  SubtractFromTree(node, delta);
}

template<typename Group, typename Base>
template<typename Node>
inline void WithDescValue<Group, Base>::AddToProperDesc(
    Node* node,
    typename Group::Type delta) {
  typedef WithDescValue Self;
  if (!node) return;
  {
    ScopedCutDirOf<Node> leaf_cut(kLeaf, node);
    if (leaf_cut.excl_part()) leaf_cut.excl_part()->Self::add_to_value(delta);
  }
  ScopedCutDashed<Node> dashed_cut(node);
  if (dashed_cut.excl_part()) {
    dashed_cut.excl_part()->Self::add_to_value(delta);
  }
}

template<typename Group, typename Base>
template<typename Node>
inline void WithDescValue<Group, Base>::SubtractFromProperDesc(
    Node* node,
    typename Group::Type delta) {
  typedef WithDescValue Self;
  if (!node) return;
  {
    ...
    if (leaf_cut.excl_part()) {
      leaf_cut.excl_part()->Self::subtract_from_value(delta);
    }
  }
  ScopedCutDashed<Node> dashed_cut(node);
  if (dashed_cut.excl_part()) {
    dashed_cut.excl_part()->Self::subtract_from_value(delta);
  }
}
...
inline Node* WithAggr<Semigroup, Base>::FindDirmostAnc(int dir,
                                                       Node* node,
                                                       Predicate predicate) {
  ScopedCutDirOf<Node> leaf_cut(kLeaf, node);
...
template<typename Semigroup, typename Base>
template<typename Node, typename Predicate>
inline Node* WithAggr<Semigroup, Base>::FindLeafmostAnc(Node* node,
                                                        Predicate predicate) {
...
template<typename Semigroup, typename Base>
template<typename Node, typename Predicate>
inline Node* WithAggr<Semigroup, Base>::FindRootmostAnc(Node* node,
                                                        Predicate predicate) {
...
class WithDescAggrOfDescValue<Semigroup, Base>::DescSearch
    : public Search<Semigroup,
                    ...
                    Predicate> {
 public:
  DescSearch(Node* node, Predicate predicate)
...
template<typename Node>
inline typename Semigroup::Type
WithDescAggrOfAncValue<Semigroup, Base>::AggrTree(Node* node) {
  typedef WithDescAggrOfAncValue Self;
  if (!node) return Semigroup::empty_aggr();
  UpdateAggrs(node->Expose());
...
template<typename Semigroup, typename Base>
template<typename Node, typename Predicate>
inline Node* WithDescAggrOfAncValue<Semigroup, Base>::FindDirmostTree(
    int dir,
    Node* node,
    Predicate predicate) {
  return (dir == kLeaf ?
...
template<typename Semigroup, typename Base>
template<typename Node, typename Predicate>
Node* WithDescAggrOfAncValue<Semigroup, Base>::FindLeafmostTree(
    Node* node,
    Predicate predicate) {
  if (!node) return NULL;
  DescSearch<Node, Predicate> search(node, predicate);
...
template<typename Semigroup, typename Base>
template<typename Node, typename Predicate>
Node* WithDescAggrOfAncValue<Semigroup, Base>::FindRootmostTree(
    Node* node,
    Predicate predicate) {
  if (!node) return NULL;
  DescSearch<Node, Predicate> search(node, predicate);
...
template<typename Node>
inline typename Semigroup::Type
WithDescAggrOfAncValue<Semigroup, Base>::AggrDesc(Node* node) {
  ScopedCutDirOf<Node> root_cut(kRoot, node);
  return AggrTree(node);
}
...
template<typename Semigroup, typename Base>
template<typename Node, typename Predicate>
inline Node* WithDescAggrOfAncValue<Semigroup, Base>::FindDirmostDesc(
    int dir,
    Node* node,
    Predicate predicate) {
  return (dir == kLeaf ?
...
template<typename Semigroup, typename Base>
template<typename Node, typename Predicate>
inline Node* WithDescAggrOfAncValue<Semigroup, Base>::FindLeafmostDesc(
    Node* node,
    Predicate predicate) {
  ScopedCutDirOf<Node> root_cut(kRoot, node);
  return FindLeafmostTree(node, predicate);
...
template<typename Semigroup, typename Base>
template<typename Node, typename Predicate>
inline Node* WithDescAggrOfAncValue<Semigroup, Base>::FindRootmostDesc(
    Node* node,
    Predicate predicate) {
  ScopedCutDirOf<Node> root_cut(kRoot, node);
  return FindRootmostTree(node, predicate);
...
template<typename Node>
inline typename Semigroup::Type
WithDescAggrOfAncValue<Semigroup, Base>::AggrProperDesc(Node* node) {
  typedef WithDescAggrOfAncValue Self;
  if (!node) return Semigroup::empty_aggr();
  ScopedCutDirOf<Node> leaf_cut(kLeaf, node);
  ScopedCutDashed<Node> dashed_cut(node);
...
template<typename Semigroup, typename Base>
template<typename Node, typename Predicate>
inline Node* WithDescAggrOfAncValue<Semigroup, Base>::FindDirmostProperDesc(
    int dir,
    Node* node,
    Predicate predicate) {
  return (dir == kLeaf ?
...
template<typename Semigroup, typename Base>
template<typename Node, typename Predicate>
Node* WithDescAggrOfAncValue<Semigroup, Base>::FindLeafmostProperDesc(
    Node* node,
    Predicate predicate) {
  if (!node) return NULL;
  DescSearch<Node, Predicate> search(node, predicate);
...
template<typename Semigroup, typename Base>
template<typename Node, typename Predicate>
Node* WithDescAggrOfAncValue<Semigroup, Base>::FindRootmostProperDesc(
    Node* node,
    Predicate predicate) {
  if (!node) return NULL;
  DescSearch<Node, Predicate> search(node, predicate);
...
template<typename Semigroup, typename Base>
template<typename Node, typename Predicate>
class WithDescAggrOfAncValue<Semigroup, Base>::DescSearch
    : public Search<Semigroup,
                    WithDescAggrOfAncValue<Semigroup, Base>,
                    Node,
                    Predicate> {
 public:
  DescSearch(Node* node, Predicate predicate)
      : Search<Semigroup, WithDescAggrOfAncValue, Node, Predicate>(node,
                                                                   predicate) {
 ...
 private:
  ...
  bool TestNonSolidOther(Node* non_solid) {
    typedef WithDescAggrOfAncValue Self;
    if (!non_solid) return false;
    if (!TestOtherAggr(non_solid)) return false;
    non_solid->DottedAddValues(this->node());
    this->set_node(non_solid);
    return true;
  }
  ...
  bool DescendDashed() {
    ...
    while (!this->Test(this->node()->Self::delta_aggr())) {
...
template<typename Group, typename Base>
template<typename Node>
inline void WithReverseBy<Group, Base>::Evert(Node* node) {
